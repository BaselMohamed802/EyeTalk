...
class Widget_AR(QWidget):
    ...
    def __init__(self):
        ...
        # 🌟 SHRUNK LOGO TO FIT ON SMALLER SCREENS 🌟
        Scaled = Thebes_Logo.scaled(250, 100, Qt.KeepAspectRatio, Qt.SmoothTransformation)
    ...
    """)
        ...
        self.text_holder_label.setWordWrap(True) # 🌟 CRITICAL: Forces long text to a new line
    ...
    """)
        ...
        # 🌟 ADDED STRETCH RATIOS HERE 🌟
        H_layout = QHBoxLayout()
        H_layout.setSpacing(20)
        H_layout.addLayout(Vertical_EM_Button, stretch=1)
        H_layout.addLayout(Vertical_TextArea, stretch=3)
        ...
        V_layout.addLayout(H_layout, stretch=3)           
        V_layout.addLayout(Keyboard_Layout_ROW1, stretch=1)  
        V_layout.addLayout(Keyboard_Layout_ROW2, stretch=1)  
        V_layout.addLayout(Keyboard_Layout_ROW3, stretch=1)  
        V_layout.addLayout(Keyboard_Layout_ROW4, stretch=1)  
        
        # Set the main layout for the widget
        self.setLayout(V_layout)

        # ============================================
        # 🌟 DYNAMIC RESIZING MAGIC LOOP 🌟
        # ============================================
        for button in self.findChildren(QPushButton):
            # Force buttons to expand and shrink dynamically
            button.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
            
            # Auto-replace hardcoded paddings so they don't break small screens
            current_style = button.styleSheet()
            new_style = current_style.replace("padding: 12px;", "padding: 3px;")
            new_style = new_style.replace("padding: 15px;", "padding: 3px;")
            new_style = new_style.replace("padding: 18px;", "padding: 3px;")
            new_style = new_style.replace("padding: 20px;", "padding: 5px;")
